...
<html lang="tr">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Sağlık Durumu - Hybrid Proxy</title>
    <style>
      body { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial; margin: 0; background: #0b1020; color: #e8eefc; }
      header { padding: 20px 24px; border-bottom: 1px solid rgba(255,255,255,0.08); background: rgba(255,255,255,0.03); }
      h1 { margin: 0; font-size: 18px; font-weight: 650; letter-spacing: 0.2px; }
...
// Protected artado page for detailed statistics with GitHub authentication
app.get('/artado', async (req: Request, res: Response) => {
  const port = Number(process.env.PORT ?? 8787);
  const health = getEngineHealth();
  const globalStats = getGlobalSearchStats();
  ...
  // Check for GitHub authentication (simple session check)
  const isAuthenticated = req.headers.authorization === 'Bearer github-sxinar-token' || 
                          req.query.token === 'github-sxinar-token';
...
<html lang="tr">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Giriş Gerekli - Artado Dashboard</title>
    <style>
  ...
  <body>
    <div class="login-card">
      <h1>🔐 Giriş Gerekli</h1>
      <p>Bu sayfaya erişim için GitHub ile giriş yapmanız gerekmektedir. Sadece sxinar kullanıcısı erişebilir.</p>
      <a href="#" onclick="window.location.href = '/artado?token=github-sxinar-token'; return false;" class="github-btn">
...
<html lang="tr">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Artado Dashboard - Detaylı İstatistikler</title>
    <style>
      body { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial; margin: 0; background: #0b1020; color: #e8eefc; }
      header { padding: 20px 24px; border-bottom: 1px solid rgba(255,255,255,0.08); background: rgba(255,255,255,0.03); }
      h1 { margin: 0; font-size: 18px; font-weight: 650; letter-spacing: 0.2px; }
  ...
  <body>
    <header>
      <h1>🎨 Artado Dashboard</h1>
      <div class="sub">Detaylı Arama İstatistikleri • Running on <span class="v">http://localhost:${port}</span> • <a href="/health">/health</a> • <a href="/">Ana Sayfa</a></div>
    </header>
    <main>
      <div class="grid">
        <section class="card third">
        ...
        <section class="card half">
          <h3 style="margin-top:0; font-size:14px; color:rgba(232,238,252,0.8); margin-bottom: 12px;">🔥 Popüler Aramalar</h3>
        ...
        <section class="card half">
          ...
          <div class="query-list">
            ${globalStats.searchQueries.slice(-20).reverse().map((query, index) => `
              <div class="query-item">
                <span class="query-text">${query}</span>
    ...
    <script>
      ...
      function renderEngines() {
        ...
        engines.forEach(engine => {
          ...
          div.innerHTML = \`
            ...
            \${h.lastErrorMessage ? \`
              <div style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 6px; padding: 8px; margin-bottom: 8px;">
                <div style="font-weight: 600; margin-bottom: 4px; font-size: 11px;">Son Hata Mesajı:</div>
                <div style="font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 10px; word-break: break-all;">\${h.lastErrorMessage}</div>
              </div>
            \` : ''}
            
            <div style="background: rgba(0,0,0,0.15); border-radius: 6px; padding: 8px; font-size: 10px;">
              <div style="color: rgba(232,238,252,0.5); margin-bottom: 4px;">Zaman Bilgileri</div>
              \${h.lastSuccess ? \`<div style="color: #46d39a; margin-bottom: 2px;">Son Başarı: \${new Date(h.lastSuccess).toLocaleString('tr-TR')}</div>\` : '<div style="color: #ffa726; margin-bottom: 2px;">Hiç başarılı işlem olmadı</div>'}
              \${h.lastError ? \`<div style="color: #ff5c7a;">Son Hata: \${new Date(h.lastError).toLocaleString('tr-TR')}</div>\` : ''}
            </div>
          \`;
          list.appendChild(div);
        });
      ...
      function updateData() {
        el('lastUpdate') && (el('lastUpdate').textContent = new Date().toLocaleString('tr-TR'));
        renderEngines();
      }
